...
This module brings together functionality of several example scripts for
solving one‑ and two‑dimensional Black-Scholes partial differential
equation (PDE) using finite differences, Parareal algorithm and
machine‑learned solution operators.  original examples were spread
...
numerical error minimisation routines defined in accompanying
...
Finite difference solvers - Functions function  solve_blackscholes_1d_fdm  and
    ...
    schemes for one‑ and two‑dimensional Black-Scholes PDE.  update
...
Parareal algorithm - function  parareal  provides a generic
    implementation of Parareal time‑parallel integration scheme.  It
    accepts user‑supplied coarse and fine propagators and iteratively
    converges to a solution on a coarse time grid.  implementation is
    agnostic to dimensionality of state and can be used for both
...
Fourier Neural Operator (FNO) models - Classes :class: FNO1D  and
    ...
    operate in Fourier domain and then project back to desired
    output.  spectral convolutions learn linear transformations on the
...
Training utilities** - helper function function  train_fno_model  trains
    ...
    progress printout.  Users can generate training data via finite
...
TensorFlow/Keras or unpublished "pararealml" modules.  functions
here can be imported and reused in other projects.

Example usage
-------------

To solve 1D Black-Scholes PDE for a European call option using the
finite difference method and compare result with a neural operator
approximation:
...
.. code-block:: python
    ...
    #Compute reference solution via finite differences
    ...
    #Train a neural operator to learn mapping from initial conditions to
    ...
    #Evaluate trained model on same input
    model.eval()
    with torch.no_grad():
...
# prevents import errors when only finite difference or Parareal
...
# Import numerical stability utilities from error_minimization module.
...
def solve_blackscholes_1d_fdm(
    ...
    """Solve 1D Black-Scholes PDE using an explicit finite difference scheme.

    This routine discretises asset price domain "[Smin, Smax]" into
    "N+1" grid points and time domain "[0, T]" into "M+1" steps.
    solution array has shape "(M+1, N+1)", where first axis
    corresponds to time ("t=0" at index 0) and second to asset
    price grid.  Boundary conditions appropriate for European call and put
    options are enforced: at "S=0" option is worthless, and at
    "S=Smax" payoff grows linearly with "S" (for a call) or is
    bounded for a put.  update formula uses central differences to
    approximate spatial derivatives and explicit Euler method for the
    temporal derivative.

    Numerical stability considerations: update formula involves
    subtracting neighbouring grid values and dividing by powers of "dS".
    To reduce effects of round‑off error differences are computed
    using function  safe_subtract  and divisions use function  safe_divide .

    initial_condition: Payoff defined on asset price grid at
        maturity "t=0".  Should be a one‑dimensional array of
        length "N+1".
    Smin: Minimum asset price in computational domain.
    Smax: Maximum asset price in computational domain.
    T: Time horizon (maturity of option).
    sigma: Volatility parameter of underlying asset.
    ...
    Returns 2D NumPy array "V" such that "V[j, i]" approx option
    ...
    # Precompute coefficients that appear in PDE to avoid recomputing
    # them in inner loop.  These depend only on spatial grid.
    sigma2 = sigma * sigma
...
def solve_blackscholes_2d_fdm(
    ...
    """Solve 2D Black-Scholes PDE using an explicit finite difference scheme.

    two‑dimensional PDE is discretised on a uniform grid of size
    "(N1+1) × (N2+1)" over rectangular domain
    "[S1min,S1max] × [S2min,S2max]".  time domain "[0, T]" is divided
    into "M+1" steps.  underlying PDE without cross‑derivative terms
    reads:
    ...
    omitted for simplicity.  Boundary conditions at edges are set
    analogously to 1D case.

    Args:
        initial_condition: Payoff defined on 2D asset price grid at
            maturity "t=0" with shape "(N1+1, N2+1)".
        S1min, S1max: Bounds of first asset price.
        S2min, S2max: Bounds of second asset price.
        T: Time horizon.
        sigma1, sigma2: Volatilities of two assets.
        r: Risk‑free interest rate.
        N1, N2: Number of grid intervals along first and second asset.
    ...
    for j in range(M):
        # Apply boundary conditions on edges of 2D grid.
        # S1 = S1min (i1=0) or S2 = S2min (i2=0): option value is zero for a call.
        if option_type == "call":
            V[j + 1, 0, :] = 0.0
            V[j + 1, :, 0] = 0.0
            # At far ends (S1max or S2max) call option payoff grows linearly.
            V[j + 1, -1, :] = S1_grid[-1]
            V[j + 1, :, -1] = S2_grid[-1]
        else:
            # For a put, value at S1=0 or S2=0 is strike price
...
def parareal(
    ...
    """Generic implementation of Parareal time‑parallel integration scheme.

    Parareal algorithm decomposes time interval "[t0, t1]" into
    ...
    propagator "fine_solver".  At each iteration coarse solution on
    each subinterval is corrected by difference between fine and
    coarse solutions from previous iteration.  method returns
    approximations "U[k]" to solution at subinterval endpoints
    after each iteration "k".

    y0: Initial state at time "t0" (could be a scalar, vector or
        multi‑dimensional array).
    t0: Start time of integration interval.
    t1: End time of integration interval.
    n_intervals: Number of coarse time subintervals to partition
        "[t0, t1]".
    fine_solver: Function that advances state from "t_start" to
        "t_end" using a fine time step.
    coarse_solver: Function that advances state from "t_start" to
    ...
    tolerance: Relative tolerance for early termination.  If the
        infinity norm of correction is below this threshold the
        algorithm stops.

    returns list of length "n_intervals+1" containing approximated
        states at subinterval endpoints after final iteration.
    """
    # Create coarse time grid.
    times = np.linspace(t0, t1, n_intervals + 1)
    # U[k][n] will hold approximate solution at time times[n] at
    # iteration k.  Start with k=0 (initial guess) and allocate storage
    # for up to max_iters iterations.  We only keep latest two
...
class SpectralConv1d(nn.Module):
    """1D spectral convolution layer used in Fourier neural operator.

    This layer performs following operations:
    1. Compute real‑valued fast Fourier transform of input along
        spatial dimension.
    2. Multiply lower "modes" Fourier coefficients by learnable
        complex weights.
    3. Inverse transform back to spatial domain.

    Only first "modes" frequencies are used; higher frequencies are
    set to zero.  weights are stored in their real and imaginary
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply spectral convolution to a batch of 1D inputs.

        x: Tensor of shape "(batch, in_channels, n)".

        returns Tensor of shape "(batch, out_channels, n)".
        """
        batchsize, channels, n = x.shape
        # Compute rFFT along last dimension; result has
        # shape (batch, channels, n//2+1) and is complex.
        x_ft = torch.fft.rfft(x)
        # Allocate zero tensor for output Fourier coefficients.
        out_ft = torch.zeros(
            batchsize,
            self.out_channels,
            x_ft.size(-1),
        ...
        # Perform multiplication on truncated modes.
        # x_ft: (batch, in_channels, n//2+1)
        # weight: (in_channels, out_channels, modes, 2)
        # We restrict to first self.modes coefficients.
        ...
        # Multiply Fourier coefficients by weights via einsum.
...
class FNO1D(nn.Module):
    ...
    architecture follows input -> lifting convolution -> 
    L × [spectral conv + linear layer] -> projection convolution -> output

    where "L" is number of layers ("num_layers").  A GELU
    nonlinearity is applied after each spectral convolution and linear
    combination.  output has same length as input and one
    channel.  This model can be used to learn mappings between functions
    defined on same 1D grid (e.g. initial condition → solution at
    maturity).
    """
...
class SpectralConv2d(nn.Module):
    ...
    This layer computes 2D Fourier transform of input, applies
    learnable complex weights on lowest "modes1" × "modes2" frequencies
    and then performs an inverse transform back to spatial domain.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply 2D spectral convolution to "x".

        x: Tensor of shape "(batch, in_channels, n1, n2)".

        returns Tensor of shape "(batch, out_channels, n1, n2)".
        """
...
class FNO2D(nn.Module):
    ...
    To keep this example focused and manageable, 2D FNO is not fully
    implemented.  You can extend :class: FNO1D  to two dimensions by
    implementing a working :class: SpectralConv2d  analogous to 1D
    version.  intent is to illustrate how to structure such a model.
    """
...
# If PyTorch is not available override FNO classes with stubs that
# raise informative errors when instantiated.  This prevents import
# errors in environments without torch while still allowing finite
...
def train_fno_model(
    ...
    inputs: Input tensor of shape "(num_samples, n)" for 1D inputs or
        ...
        added internally.
    targets: Target tensor of same shape as "inputs" (for 1D) or
...
def parareal_blackscholes_1d(
    ...
    """Solve 1D Black-Scholes PDE using Parareal.
    ...
    user specifies fine and coarse time step sizes "dt_fine" and
    "dt_coarse"; solver functions compute solution over each
    subinterval "[t_n, t_{n+1}]".  Parareal algorithm then
    orchestrates these propagators to produce a parallelised solution.

    initial_condition: Array of length "N+1" representing payoff
        at maturity.
    Smin, Smax, T, sigma, r, N: Same as for
        function  solve_blackscholes_1d_fdm .
    n_intervals: Number of coarse time subintervals in Parareal
        decomposition.
    dt_fine: Fine time step used by fine propagator.
    dt_coarse: Coarse time step used by coarse propagator.
    max_iters: Maximum number of Parareal iterations.
    tolerance: Convergence tolerance.
    option_type: ""call"" or ""put"".

    Returns list of length "n_intervals+1" containing approximate
        solution at subinterval endpoints after convergence.
    """
    ...
    # Fine solver: solve PDE on a subinterval [t_start, t_end] using dt_fine.
...
def parareal_blackscholes_2d(
    ...
    """Solve 2D Black-Scholes PDE using Parareal.

    This is analogous to function  parareal_blackscholes_1d  but operates on
    two‑dimensional grids.  fine and coarse propagators are built from
    ...
    Returns list containing approximate solution at subinterval
